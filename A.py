...
def SolveA(pattern, testCase, heuristic):
    beg = float(time.time())
    fuel = getFuel(pattern)
    file_name = f"a-h{heuristic}-sol-{testCase}.txt"
    ...
    for i in fuel:
        ...
        start = 1
    file.write("\n")
    file_name = f"a-h{heuristic}-search-{testCase}.txt"
    H = open(file_name, "w")
    Q = [pattern]
    st = {}
    ...
    while len(Q) > 0:
        ...
        p = Q[0]
        dist = st[getGrid(p)]
        updated_heuristic_value = calculate_heuristic(p, heuristic)
        H.write(f"{updated_heuristic_value + dist} {dist} {updated_heuristic_value} {p}\n")
        if p[17] == 'A':
            finalState = p
